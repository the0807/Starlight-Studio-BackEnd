...
6. 내용에서 캐릭터가 말하는 대사는 큰따옴표로 표시할 것
7. 단어는 10세 이하 어린이가 이해할 수 있는 쉬운 단어를 사용할 것
8. 문장은 '~했어요'와 같은 문장으로 만들 것
    """
    
    #input 지정
    prompt = f"###페이지 번호: {num}\n\n###주제: {topic}\n\n###캐릭터: {character}\n\n###배경: {background}\n\n###이전내용: {context}\n###참고내용: {request_text}\n\n"
    prompt += "###사용자 입력: 페이지 번호에 해당하는 내용을 만드는 건데, 수정 문장을 내가 만든 페이지 내용인데, 문장을 수정하고 싶어.\n너는 주제, 캐릭터, 배경, 이전 내용을 참고해서 다음 동화 내용을 만들면 되는데, 참고내용을 적용해서 이전 내용의 다음 내용 한 문장을 만들어줘."
